...
class ItcSubmitForm {
  ...
  constructor(target, config = {}) {
    ...
    this._isCheckValidationOnClient =
      config["isCheckValidationOnClient"] !== false;
    this._elForm = target;
    this._init();
  }
  ...
  _setStateValidaion(input, state, message) {
    ...
    input
      .closest(".form-group")
      .querySelector(".invalid-feedback").textContent = "";
    if (state === "error" || state === "success") {
      input.classList.add(className);
      input
        .closest(".form-group")
        .querySelector(".invalid-feedback").textContent = text;
    }
  }
  ...
  _checkValidity() {
    ...
    this._attach.items.forEach((item) => {
      const elAttach = this._elForm.querySelector(
        '.form-attach__item[data-index="' + item.index + '"]'
      );
      if (item.file.size > this._attach.maxFileSize * 1024) {
        this._setStateValidaion(
          elAttach,
          "error",
          `Размер файла больше ${this._attach.maxFileSize}Кб`
        );
        valid = false;
      } else if (
        !this.constructor._checkExt(item.file.name, this._attach.ext)
      ) {
        this._setStateValidaion(
          elAttach,
          "error",
          "Тип не является допустимым"
        );
        valid = false;
      } else {
  ...
  _successXHR(data) {
    ...
    if (data["result"] === "success") {
      this._elForm.dispatchEvent(
        new Event("itc.successSendForm", { bubbles: true })
      );
      return;
    }
    ...
    if (!Object.keys(data["errors"]).length) {
      this._elForm.querySelector(".form-error").textContent =
    ...
    this._elForm
      ...
      .classList.remove("form-error_hide");
  ...
  _onSubmit() {
    ...
    const submitWidth = this._elForm
    ...
    this._elForm.querySelector(
      '[type="submit"]'
    ).style.width = `${submitWidth}px`;
    this._elForm.querySelector(
      '[type="submit"]'
    ).style.height = `${submitHeight}px`;
  ...
  _init() {
    ...
    elFormAttachCount
      ? (elFormAttachCount.textContent = this._attach.maxItems)
      : null;
    this._submitText =
      this._elForm.querySelector('[type="submit"]').textContent;
        ...
        if (file.type.match(/image.*/)) {
          ...
          reader.addEventListener("load", (e) => {
            this._elForm.querySelector(".form-attach__items").innerHTML +=
              this.constructor._getAttachTemplate(index, file, e.target);
          });
        } else {
          this._elForm.querySelector(".form-attach__items").innerHTML +=
            this.constructor._getAttachTemplate(index, file);
        }
      }
      target.value = "";
    });
